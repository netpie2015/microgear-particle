...
bool MicroGear::publish(char* topic, char* message, bool _retained) {
    char top[MAXTOPICSIZE] = "/";

    strcat(top,appid);
    strcat(top,topic);
    return mqttclient->publish(top, message, _retained);
}
...
bool MicroGear::publish(char* topic, double message, int n, bool _retained) {
    char mstr[16];
//    dtostrf(message,0,n,mstr);
    return publish(topic, mstr, _retained);
}
...
bool MicroGear::publish(char* topic, double message, bool _retained) {
    return publish(topic, message, 8, _retained);
}
...
bool MicroGear::publish(char* topic, int message, bool _retained) {
    return publish(topic, message, 0, _retained);
}
...
bool MicroGear::publish(char* topic, String message, bool _retained) {
    char buff[MAXBUFFSIZE];
    message.toCharArray(buff,MAXBUFFSIZE-1);
    return publish(topic, buff, _retained);
}
